...
5. 将摄像头对准 A 设备显示的二维码：
   - **单二维码**：扫一次即完成
   - **分块二维码**：按 A 设备展示顺序依次扫描全部 N 帧，底部进度条显示"已扫描 X/N"；扫错或想重来时可点击 **重新开始** 清空已扫帧重新扫描
...
| 扫到畸形/无效的分块二维码（缺字段） | 显示"无效的二维码，请对准有效的分块二维码"提示，忽略该帧 | 继续扫描正常帧，不中断已累积进度 |
| 分块模式下扫到错误的帧 | 显示"第 X 帧校验失败" | 重新扫描该帧 |
| 分块模式下帧序不完整 | 进度条显示"已扫描 X/N"，无法完成 | 补扫缺失的帧，或点击 **重新开始** 清空重扫 |
| 分块组装失败（校验不通过） | 自动清空已累积帧并提示错误 | 重新进入扫码 Tab 重新扫描 |
