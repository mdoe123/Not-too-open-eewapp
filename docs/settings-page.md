...
| 卡片 | 跳转路由 | 说明 |
|------|----------|------|
| 模拟预警 | `SimulateAlert` | 配置并触发模拟地震预警测试 |
| 关于 | `About` | 应用信息、用户协议、开源许可（详见 `docs/onboarding.md` 第 9 节） |

### 8.2 模拟预警页面（SimulateAlertScreen）

**震中坐标计算**：基于用户当前位置（GPS 定位或手动输入）+ 震中距 + 随机方位角反算虚拟震中坐标，不再使用硬编码的北京坐标。这样模拟预警的震中距与用户实际位置匹配，悬浮窗倒计时和烈度估算更真实。

**震中名称**：固定显示「模拟预警」，不带地名后缀（之前为"模拟震中（距北京${distance}km）"，在用户不在北京时会产生误导）。

**与真实事件的关系**：
- 模拟事件 ID 前缀为 `simulate:`，真实事件 ID 前缀为 `customSource-`，两者不会冲突
- 模拟事件不会覆盖真实事件，反之亦然
- 模拟事件 `originTime = Date.now()`（最新），按 `originTime` 降序排列时会出现在列表顶部，这是正常排序行为，不是覆盖
- 跨源去重（`dedupKey`）要求 `originTime + 坐标(0.01度) + 震级(0.1级)` 完全相同才触发，模拟事件与真实事件几乎不可能命中此条件
