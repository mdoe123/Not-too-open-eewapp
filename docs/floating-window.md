...
支持**多事件并发显示**（最多 3 个事件上下垂直排列）：大震独占显示直到倒计时归零 60 秒后，同级别地震垂直并列。前台悬浮窗、后台悬浮窗、锁屏预警 Activity 三条路径全部支持多事件并发显示。详见 [多事件并发显示规则](#多事件并发显示规则)。
...
- `remainSec > 0`：正常倒计时显示，警报持续；语音播报规则：
  - 整十数（10/20/30/40/50/60/70/80/90）：`[级别] + [中文整十读法] + ["秒后抵达"]`
    - 10 → "地震红色预警" + "十" + "秒后抵达"
    - 20 → "地震红色预警" + "二" + "十" + "秒后抵达"
    - 60 → "地震红色预警" + "六" + "十" + "秒后抵达"
  - 非整十数（如 64/23/15）：`[级别] + [数字逐位]`（不加"秒后抵达"）
    - 64 → "地震红色预警" + "六" + "四"
    - 23 → "地震红色预警" + "二" + "三"
...
EewBackgroundService（原生 ForegroundService）
  ...
  └─ startLockScreenActivity()（启动 LockScreenAlertActivity，传入 sound/vibration/flashlight 配置）
       ...
       └─ LockScreenAlertActivity（独立 Activity，显示在锁屏界面之上，支持多事件垂直排列）
            ...
            ├─ stopAlerts()              停止声音/震动/闪光灯（所有事件到达后 -60 秒或 onDestroy 时）
...
1. **候选过滤**：未被用户手动关闭 + `remain > -60`（倒计时归零后 60 秒内仍算活跃）；取消报（`isCancel=true`）不受此限制
2. **排序**：按预警级别降序（red > orange > yellow > blue），同级别按预估烈度降序
3. **顶级事件**：候选中级别最高的那一个，显示在最上方
4. **并列事件**：与顶级**同级别**（差 0 档）的其他事件，最多 2 个，显示在下方
5. **大小压制**：差 ≥ 1 档的事件被顶级"压制"不显示，等顶级 `remain <= -60` 后才让下一级显示
...
| 大震归零后 59s | -59s | 1s | 仅大震（仍独占显示） |
| 大震归零后 60s | -60s | 0s | 大震被过滤，显示小震（如果还有倒计时） |
| 大震归零后 61s | -61s | -1s | 小震也被过滤，无显示 |
...
| `ALERT_CONTINUE_AFTER_ARRIVAL_SEC` | -60 | 倒计时归零后继续显示/警报的秒数阈值 |
