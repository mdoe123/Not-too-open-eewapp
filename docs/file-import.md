...
4. **一键导入全部**：
   ...
   - 合并到现有源列表（按 `priority` 去重，同 `priority` 覆盖）
5. **手动启用**：
   - 导入后所有源默认禁用
   - 在数据源管理列表中手动启用需要的源
...
- **去重键**：`priority` 字段
- **同 priority 源**：覆盖现有源配置
- **新 priority 源**：追加到列表末尾
- **强制禁用**：导入的源 `enabled` 强制设为 `false`，需用户手动启用
- **结果统计**：返回 `added`（新增数量）和 `updated`（覆盖数量）
